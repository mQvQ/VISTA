...
def construct_interaction_image_KNN(adata, n_neighbors=3):
    feats = adata.obsm['img_ebd']
    n_spot = feats.shape[0]
    # 基于feats 的距离计算neighbors, 余弦相似度 + 暴力搜索
    nbrs = NearestNeighbors(n_neighbors=n_neighbors + 1, metric='cosine', algorithm='brute').fit(feats)
