...
class STACI(nn.Module):
    def __init__(self, input_feat_dim, hidden_dim1, hidden_dim2, hidden_decoder, dropout, meanMin=1e-5, meanMax=1e6,
                 thetaMin=1e-5, thetaMax=1e6):
        ...
        self.theta = FC(hidden_decoder, input_feat_dim, dropout=0,
                        act=lambda x: torch.clamp(F.softplus(x), min=thetaMin, max=thetaMax), batchnorm=False,
                        bias=True)
        self.mean = FC(hidden_decoder, input_feat_dim, dropout=0,
                       act=lambda x: torch.clamp(torch.exp(x), min=meanMin, max=meanMax), batchnorm=False, bias=True)
...
class GCNModelVAE_FC(nn.Module):
    def __init__(self, in_features, hidden_dim, out_features, hidden_decoder, dropout, meanMin=1e-5, meanMax=1e6,
                 thetaMin=1e-5, thetaMax=1e6):
...
class Decoder(nn.Module):
    def __init__(self, in_features, out_features, hidden_decoder, dropout, meanMin, meanMax, thetaMin, thetaMax):
        ...
        self.theta = GraphConvolution(hidden_decoder, in_features, dropout=0,
                                      act=lambda x: torch.clamp(F.softplus(x), min=thetaMin, max=thetaMax))
        self.mean = GraphConvolution(hidden_decoder, in_features, dropout=0,
                                     act=lambda x: torch.clamp(torch.exp(x), min=meanMin, max=meanMax))
...
class GCNModelVAE_IMG(nn.Module):
    def __init__(self, in_features, hidden_dim, out_features, outputshape, dropout, meanMin=1e-5, meanMax=1e6,
                 thetaMin=1e-5, thetaMax=1e6):
...
class IMG_Decoder(nn.Module):
    def __init__(self, out_features, outputshape, dropout, meanMin, meanMax, thetaMin, thetaMax):
        ...
        self.gc3 = GraphConvolution(out_features, outputshape, dropout=0,
                                    act=lambda x: torch.clamp(torch.exp(x), min=meanMin, max=meanMax))
