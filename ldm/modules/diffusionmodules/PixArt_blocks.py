...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        if mask is not None:
            # only support consist mask for a batch
            trg_num, src_num = mask.shape[1], mask.shape[2]
            attn_bias = torch.zeros([1, self.num_heads, q.shape[1], k.shape[1]], dtype=q.dtype, device=q.device)
            mask = mask.unsqueeze(1).repeat(B, self.num_heads, 1, 1)
            mask = mask.view(1, -1, trg_num * B, src_num * B)
        ...
        x = out.contiguous().reshape(B, -1, C)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class STT2IFinalLayer(nn.Module):
    """
    The final layer of PixArt.
    """

    def __init__(self, hidden_size, gene_num, out_channels):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.linear = nn.Linear(hidden_size, gene_num * out_channels, bias=True)
        self.scale_shift_table = nn.Parameter(torch.randn(2, hidden_size) / hidden_size ** 0.5)
        self.out_channels = out_channels
