...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        if gen_joint:
            self.model = JointDiffusionWrapper(unet_config, conditioning_key)
        else:
    ...
    def register_schedule(
        ...
        # to_torch = partial(torch.tensor, dtype=torch.float16) # add by mt
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        # model_out = self.model(x_noisy.to(torch.float16), t)
...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox", 'conds', 'rna']:
                    xc = batch[cond_key]
    ...
    def shared_step(self, batch, **kwargs):
        ...
        # print('ddpm shared step', x.dtype, c.dtype) # float16, float16
        loss = self(x, c)
        return loss
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c) # list
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        # x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise).to(torch.float16)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        # print('check x_noisy', x_noisy.dtype)
        ...
        if self.parameterization == "x0":
            ...
            raise NotImplementedError()
        # print('check output and target', model_output.dtype, target.dtype)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        # print('check forward diff', x.dtype)
...
class JointDiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model = instantiate_from_config(diff_model_config)
        self.conditioning_key = conditioning_key
    ...
    def init_from_ckpt_freeze_image(self, path):
        sd = torch.load(path, map_location="cpu")['state_dict']
        if "state_dict" in list(sd.keys()):
            sd = sd["state_dict"]
        keys = list(sd.keys())
        # only load image weights
        img_params = []
        for k in keys:
            if 'image' in k or 'vx' in k or 'vh' in k:
                img_params.append(k)
            else:
                if 'diffusion' in k:
                    print("Deleting key {} from state_dict.".format(k))
        ...
        for name, param in self.named_parameters():
            print(name)
            if 'image' in name or 'vx' in name or 'vh' in name:
                param.requires_grad = False
                print(f'{name} is frozen, {param.requires_grad}')


    def forward(self, v_x, st_x, t, c_concat: list = None, c_crossattn: list = None):
        # print('check forward diff', x.dtype)
        # x = x.to(torch.float16)
        if self.conditioning_key is None:
            v_out, st_out = self.diffusion_model(v_x, st_x, t)
        elif self.conditioning_key == "crossattn":
            cc = torch.cat(c_crossattn, 1)
            v_out, st_out = self.diffusion_model(v_x, st_x, t, context=cc)
